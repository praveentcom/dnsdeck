...
final class Route53Service {
    ...
    func listHostedZones() async throws -> [R53HostedZone] {
        ...
        repeat {
            ...
            var queryItems: [URLQueryItem] = [URLQueryItem(
                name: "maxitems",
                value: "\(Constants.Pagination.route53PageSize)"
            )]
            if let marker {
    ...
    func listResourceRecordSets(hostedZoneId: String) async throws -> [R53ResourceRecordSet] {
        ...
        repeat {
            var components =
                URLComponents(string: "\(baseURL)/2013-04-01/hostedzone/\(cleanZoneId(hostedZoneId))/rrset")!
            var queryItems: [URLQueryItem] = [URLQueryItem(
                name: "maxitems",
                value: "\(Constants.Pagination.route53MaxPageSize)"
            )]
    ...
    private func encodeToXML(_ object: some Encodable) throws -> Data {
    ...
    private func parseResourceRecordSetsXML(_ xml: String) -> [String: Any] {
        ...
        for match in matches {
            if let range = Range(match.range(at: 1), in: xml) {
                ...
                let rrRegex = try! NSRegularExpression(
                    pattern: resourceRecordPattern,
